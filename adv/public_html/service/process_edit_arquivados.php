...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $updateQuery = "UPDATE arquivados SET nome=?, matricula=?, processo=?, dataatual=?, selecao=?, unidade=?, anotacoes=? WHERE id=?";
    ...
    if ($stmtUpdate) {
        ...
        if (!$resultUpdate) {
            ...
            mysqli_close($conn);
            header("Location: ../routers/arquivados.php"); // Substitua "index.php" pelo nome da sua página principal
            exit();
        }
    } else {
